...
/**
 * Return text that the given font can measure/draw without throwing.
 * Standard PDF fonts only support Latin (WinAnsi); any character outside
 * the font's charset (e.g. Sinhala/Tamil) is dropped so PDF generation
 * never crashes, falling back to the given default when nothing is left.
 */
function safeText(text, font, fallback) {
  try {
...
/**
 * Render the certificate PDF and return the raw bytes (no upload).
 * @param {string} [options.fallbackStudentName] - Shown when the student's
 *   name has no characters the PDF fonts can render (e.g. their email)
...
export async function renderCertificatePDF({
  ...
  // Handwriting font for the instructor's signature; falls back to italic
  // if the font file can't be loaded
  let signatureFont = timesItalicFont
  try {
    pdfDoc.registerFontkit(fontkit)
    const fontBytes = await readFile(
      path.join(process.cwd(), "lib/fonts/GreatVibes-Regular.ttf")
    )
    signatureFont = await pdfDoc.embedFont(fontBytes, { subset: true })
  } catch (error) {
    console.error("[Certificate] Could not load signature font:", error)
  }

  const nameFont = timesBoldFont
  const courseFont = timesBoldFont
  const instructorFont = timesItalicFont
  const orgFont = timesBoldFont

  const orgName = safeText(organizationName, orgFont, "E-Pencil")
  const displayName = safeText(studentName, nameFont, safeText(fallbackStudentName, nameFont, "Student"))
  const displayCourse = safeText(courseTitle, courseFont, "Course")
  const displayInstructor = safeText(instructorName, instructorFont, "Course Instructor")
  ...
  const orgNameWidth = orgFont.widthOfTextAtSize(orgName, orgNameSize)
  ...
  let nameSize = 42
  let nameWidth = nameFont.widthOfTextAtSize(displayName, nameSize)

  // Auto-adjust name size if too long
  if (nameWidth > maxNameWidth) {
    nameSize = Math.max(28, (maxNameWidth / nameWidth) * 42)
    nameWidth = nameFont.widthOfTextAtSize(displayName, nameSize)
  }
  ...
  let courseSize = 26
  let courseWidth = courseFont.widthOfTextAtSize(displayCourse, courseSize)

  if (courseWidth > maxCourseWidth) {
    courseSize = Math.max(20, (maxCourseWidth / courseWidth) * 26)
    courseWidth = courseFont.widthOfTextAtSize(displayCourse, courseSize)
  }
  ...
  // Instructor signature on right: handwritten-style signature above the
  // line, printed name and label below it
  ...
  const signatureText = safeText(displayInstructor, signatureFont, "Instructor")
  const signatureSize = 26
  const signatureWidth = signatureFont.widthOfTextAtSize(signatureText, signatureSize)

  page.drawText(signatureText, {
    x: instructorX + (sigLineWidth - signatureWidth) / 2,
    y: sigY + 6,
    size: signatureSize,
    font: signatureFont,
    color: rgb(0.1, 0.15, 0.35), // Ink blue
  })
  ...
  const instructorNameWidth = instructorFont.widthOfTextAtSize(displayInstructor, 11)
  page.drawText(displayInstructor, {
...
export async function generateCertificatePDF(options) {
  try {
    const pdfBytes = await renderCertificatePDF(options)
    const fileKey = `certificates/${options.certificateId}.pdf`
