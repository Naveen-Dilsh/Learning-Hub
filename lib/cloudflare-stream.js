...
export async function initiateTusUpload(metadata, accountId, apiToken) {
  // Build allowed origins: production domain + localhost for dev
  const productionUrl = process.env.NEXTAUTH_URL || ""
  const allowedOrigins = [
    productionUrl,
    productionUrl.replace("https://www.", "https://"),  // also bare domain
    "http://localhost:3000",
  ].filter(Boolean).join(",")

  // Include allowedorigins in metadata — required for CORS on direct browser uploads
  const fullMetadata = { ...metadata, allowedorigins: allowedOrigins }

  const tusMetadata = Object.entries(fullMetadata)
    .map(([key, value]) => {
  ...
  // direct_user=true + allowedorigins metadata = CORS-enabled TUS URL for browser uploads
  const response = await fetch(
