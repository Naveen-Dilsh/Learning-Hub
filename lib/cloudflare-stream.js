...
export async function initiateTusUpload(metadata, accountId, apiToken) {
  // Only encode fields Cloudflare's TUS API accepts in Upload-Metadata
  // Invalid fields (filetype, uploadSize etc.) cause a 400 Bad Request
  const VALID_TUS_METADATA_KEYS = ["name", "requiresignedurls"]
  const tusMetadata = Object.entries(metadata)
    .filter(([key]) => VALID_TUS_METADATA_KEYS.includes(key))
    .map(([key, value]) => {
  ...
  // Upload-Length = total file size (sent as header, NOT in metadata)
  const uploadLength = metadata.uploadSize

  // direct_user=true → issues a browser-safe TUS upload URL
  const response = await fetch(
...
export async function deleteVideoFromCloudflare(videoId, accountId, apiToken) {
  if (!videoId || !accountId || !apiToken) {
    throw new Error(`Missing required parameters: videoId=${!!videoId}, accountId=${!!accountId}, apiToken=${!!apiToken}`)
  }

  console.log("[v0] Deleting video from Cloudflare Stream:", {
    videoId,
    accountId: accountId.substring(0, 8) + "...",
  })
  ...
  try {
    ...
    if (!response.ok) {
      // If video not found in Cloudflare (404), it's okay - might have been deleted already
      if (response.status === 404) {
        console.log("[v0] Video not found in Cloudflare Stream (may have been deleted already):", videoId)
        return { success: true, alreadyDeleted: true }
      }

      console.error("[v0] Cloudflare delete API error:", {
        status: response.status,
        error: data,
      })
      throw new Error(`Cloudflare API error: ${data.errors?.[0]?.message || 'Unknown error'}`)
    ...
    if (!data.success) {
      console.error("[v0] Invalid Cloudflare delete response:", data)
      throw new Error("Invalid delete response from Cloudflare")
    }

    console.log("[v0] Successfully deleted video from Cloudflare Stream:", videoId)
    return { success: true, alreadyDeleted: false }
  } catch (error) {
    console.error("[v0] Error deleting video from Cloudflare:", error.message)
    throw error
  }
}
