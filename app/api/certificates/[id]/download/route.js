...
export async function GET(request, { params }) {
  try {
    ...
    // Get certificate with instructor information
    ...
    if (!certificate.certificateUrl) {
      ...
      try {
        // Generate certificate PDF with instructor name
        const instructorName = certificate.course.instructor.name || 
                              certificate.course.instructor.email.split('@')[0]
        ...
        const fileKey = await generateCertificatePDF({
          ...
          instructorName: instructorName, // ✅ Add instructor name
          organizationName: "E-Pencil", // ✅ Optional: customize your org name
        })
    ...
    try {
      ...
      if (error.name === "NotFound" || error.$metadata?.httpStatusCode === 404) {
        console.error(`[Certificate] File not found in R2: ${fileKey}. Regenerating...`)
        
        // Regenerate certificate if file not found
        try {
          const instructorName = certificate.course.instructor.name || 
                                certificate.course.instructor.email.split('@')[0]
          ...
          // Update certificate with new file key
          await prisma.certificate.update({
            where: { id: certificateId },
            data: {
              certificateUrl: newFileKey,
            },
          })
          ...
          console.error(`[Certificate] Error regenerating certificate:`, regenError)
          return NextResponse.json(
            { 
              error: "Certificate file not found and regeneration failed.",
              certificateId: certificateId,
            },
            { status: 500 }
          )
